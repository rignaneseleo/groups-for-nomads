...
def generate_markdown(data):
    ...
    md_content = "[//]: # THIS FILE IS GENERATED. DO NOT EDIT THIS FILE, EDIT THE YAML INSTEAD\n\n"

    # Add last update date and total groups count
    total_groups = len(data.get("groups", []))
    last_update = datetime.now().strftime("%B %d, %Y")
    md_content += f"*Last updated: {last_update}*\n"
    md_content += f"*Total groups: {total_groups}*\n\n"
    ...
    for group in world_groups:
        ...
        # Add tags if present
        tags = group.get("tags", [])
        tag_string = " " + " ".join([f"#{tag}" for tag in tags]) if tags else ""

        if icon:
            md_content += f"![{platform.capitalize()}]({icon}) "
        md_content += f"[{name}{language}{commercial}]({url}){tag_string}\n\n"
                ...
                for group, location in city_groups[city_key]:
                    if (
                        ...
                        if icon:
                            md_content += f"![{platform.capitalize()}]({icon}) "
                        md_content += (
                            f"[{name}{language}{commercial}]({url}){tag_string}\n\n"
                        )
